...
# Cache for analysis results (10-minute TTL)
analysis_cache = TTLCache(maxsize=100, ttl=600)

def get_cached_analysis(content: str, url: str) -> Optional[Dict[str, Any]]:
    cache_key = f"{content[:100]}:{url}"
    return analysis_cache.get(cache_key)

def set_cached_analysis(content: str, url: str, result: Dict[str, Any]):
    cache_key = f"{content[:100]}:{url}"
    analysis_cache[cache_key] = result

# Helpers
nltk.download("stopwords", quiet=True)
nltk.download("punkt", quiet=True)  # Required for LexRank
STOPWORDS = set(nltk.corpus.stopwords.words("english"))
...
@lru_cache(maxsize=1)
def sum_pipe():
    return None  # Not used; LexRank is faster

@lru_cache(maxsize=1)
def emo_pipe():
    return None  # Not used; TextBlob is faster

# Feature functions
def extract_keywords(text: str, top_n: int = 3) -> List[Dict[str, Any]]:
    plain = strip_html(text)[:2000]  # Limit to 2000 characters
    phrases = kw_model().extract_keywords(
        plain, keyphrase_ngram_range=(1, 2), stop_words="english", top_n=top_n
    )
    return [{"keyword": k, "relevance": round(s, 3)} for k, s in phrases]
...
def grammar_and_spelling(text: str) -> str:
    plain = strip_html(text)[:1000]
    corrected = str(TextBlob(plain).correct())
    return html_breaks(corrected) if corrected.strip() != plain.strip() else plain
...
def sentiment_analysis(text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "emotions": {
            "Positive": 0.7 if pol > 0.05 else 0.0,
            "Neutral": 0.2 if abs(pol) <= 0.05 else 0.0,
            "Negative": 0.1 if pol < -0.05 else 0.0
        }
    }
